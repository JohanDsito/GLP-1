...
const priceId = Deno.env.get('MUSCLE_PLAN_PRICE_ID') ?? '';
...
Deno.serve(async (request) => {
  ...
  if (!stripeSecretKey || !priceId || !supabaseUrl || !supabaseAnonKey || !appUrl) {
    return new Response('Missing Stripe or Supabase environment variables', { status: 500, headers: corsHeaders });
  }
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { user_id: user.id, product: 'muscle_plan' },
      success_url: `${appUrl}/muscle-plan/quiz?payment=success`,
      cancel_url: `${appUrl}/muscle-plan/upgrade`,
    });
